...
public final class StubAnalysisResult implements AnalysisResult {

    private final AnalysisResultEntry<Object> analysisResult;

    public StubAnalysisResult(AnalysisResultEntry<Object> analysisResult) {
    ...
    public <T> Set<AnalysisResultEntry<T>> getResultEntries(Class<T> type) {
        return new HashSet<AnalysisResultEntry<T>>(Arrays.asList((AnalysisResultEntry<T>) this.analysisResult));
    }

    @Override
    public Set<FileSystemEntry> getFileSystemEntries() {
        return new HashSet<FileSystemEntry>(Arrays.asList(this.analysisResult.getFileSystemEntry()));
    }
    ...
    public Set<Class<?>> getResultTypes() {
        return new HashSet<Class<?>>(Arrays.asList(Object.class));
    }
}
